...
重要ルール：
- nameは商品の本質部分のみを、レシートの印字どおり一字一句正確にコピーする。残す部分の要約・翻訳・変換は禁止
【品目名の整形（本質部分の抜き取り）】
- 行頭の管理番号・商品コード・JANコードなどの数字列（例：「4901306024 カゴメトマトジュース」の数字部分）、「※」「★」「軽」「#」などの記号は品目名に含めない（記号は税区分の判定にのみ使う）
- 規格・容量・入数・キャッチコピーなどの付帯情報は省く。例：
  ・「カゴメトマトジュース　食塩無添加　200ml×6」→「カゴメトマトジュース」
  ・「コカ・コーラ 500mlPET お買得」→「コカ・コーラ」
  ・「軍手 12双組 徳用」→「軍手」
- ただし省いた後に残す部分の文字は印字どおり正確に。商品名そのものを短縮・言い換えしてはいけない（「カゴメトマトジュース」を「トマトジュース」にしない）
- Uberのレシート（店名がUber・Uber Trip等）の場合、品目名は「メーター料金他」で固定してよい。accountは「タクシー代」
...
{"receipts":[{"store_name":"店名","invoice_number":"T+13桁またはnull","invoice_status":"適格または要確認","date":"YYYY-MM-DD","car_number":"自動車登録番号またはnull","tax_type":"内税または外税","items":[{"name":"印字文字そのまま","amount":数値(値引きは必ず負の数),"tax_category":"10%標準または8%軽減または非課税または不課税","account":"駐車代/タクシー代/飲料代/飲食代/消耗品/自賠責保険/雑費のいずれか"}],"taxable_8":数値またはnull,"taxable_10":数値またはnull,"tax_8":数値またはnull,"tax_10":数値またはnull,"total":数値}]}` }
          ]
        }]
      })
    });
