...
- tax_category（品目の税区分）は次の4区分のいずれかで判定する：「10%標準」「8%軽減」「非課税」「不課税」
  ・まず内容で判定する（マークより内容を優先）：
    - 自賠責保険料・各種保険料 → 「非課税」
    - 切手・郵便切手・レターパック・ゆうパック等の郵送料 → tax_categoryは「10%標準」（本来は非課税だが経理処理の都合で10%課税として扱う）、accountは「切手・郵送代」
...
- invoice_numberは「T」で始まり数字13桁が続く番号（例：T2010001098023）。レシートの隅・左下・欄外・薄い小さな文字でも必ず探して読み取る。
  ...
  ・どうしても読み取れない場合のみnull。
- phone：レシートに印字された店舗の電話番号（TEL・☎の近く）。ハイフン付きでそのまま読み取る（例：「0470-22-3733」「03-3675-4649」）。なければnull。店舗の特定に使うため、市外局番から正確に読むこと
